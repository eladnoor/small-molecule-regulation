...
\end{eqnarray}
In order to compare this result to the irreversible case (in Equation \ref{eq:eps_s_v_irr}), we note first that $\gamma < 1$ (and therefore $\gamma^{-1} > 1$) and also that $\kappa =  \frac{s/K_S}{1 + s/K_S + p/K_P} < \frac{s/K_S}{1 + s/K_S}$. Therefore, we prove that
\begin{eqnarray}
\epsilon_{s,\text{reversible}}^v = \gamma^{-1} - \kappa > 1 - \kappa > 1 - \frac{s}{K_M + s} = \epsilon_{s,\text{irreversible}}^v
\end{eqnarray}
and furthermore, $\epsilon_{s,\text{reversible}}^v \rightarrow \epsilon_{s,\text{irreversible}}^v$ as $p \rightarrow 0$.
