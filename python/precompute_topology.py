...
S_sign = S.drop(met2remove) != 0

n_met, n_rxn = S_sign.shape
# build the adjacency matrix for the bipartite graph
# where the first n_met nodes are the metabolites and the last n_rxn
# are the reactions
A = np.zeros((n_met+n_rxn, n_met+n_rxn))
A[0:n_met, n_met:] = S_sign
A[n_met:, 0:n_met] = S_sign.transpose()
...
# Get unique groups
smrn = smrn_temp.groupby(['bigg.metabolite',
                          'bigg.reaction']).first().reset_index()
...
for ii in smrn.index:
    ...
    # divide by 2 to account for bipartite graph distance
    smrn.at[ii, 'SPL'] = spl[metsource][rxntarget]
